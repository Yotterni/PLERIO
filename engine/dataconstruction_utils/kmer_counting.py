...
class KmerCounter:
    ...
    representations.
    """
    def __init__(self, ks: list[int], stride: int | list[int] = 1,
                 mode: str = 'soft', rna_alphabet: bool = False,
                 torch_output: bool = False) -> None:
        """
    ...
    def __call__(self, seq: str) -> np.ndarray[float] | torch.Tensor:
        """
        A high-level API for class insides. Calculates a
    ...
    def create_template_kmer_dict_(self, k: int, default_count: int = 0
                                   ) -> dict[str, int]:
        """
    ...
    def single_k_calculator_(self, k_value: int, seq: str
                             ) -> np.ndarray[float]:
        """
    ...
    def multi_k_calculator_(self, seq: str
    ...
    def __str__(self) -> str:
        return f'KmerProсessor(ks={list(self.ks_to_templates)})'
